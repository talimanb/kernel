...
static int es8323_set_gpio(int gpio, bool level)
{
	...
	DBG("%s : set %s %s ctl gpio %s\n", __func__,
	    ...
	    gpio & ES8323_CODEC_SET_HP ? "hp" : "", level ? "HIGH" : "LOW");
...
static irqreturn_t hp_det_irq_handler(int irq, void *dev_id)
{
	...
	type =
	    gpio_get_value(es8323->
			   hp_det_gpio) ? IRQ_TYPE_EDGE_FALLING :
	...
	if (mute_flag == 0) {
		if (es8323->hp_det_level == gpio_get_value(es8323->hp_det_gpio)) {
			...
			es8323_set_gpio(ES8323_CODEC_SET_HP,
					es8323->hp_gpio_level);
		} else {
			    ...
			    const struct i2c_device_id *id)
{
	...
	es8323 =
	    devm_kzalloc(&i2c->dev, sizeof(struct es8323_priv), GFP_KERNEL);
	if (es8323 == NULL)
		return -ENOMEM;
	...
	es8323->spk_ctl_gpio =
	    of_get_named_gpio_flags(i2c->dev.of_node, "spk-con-gpio", 0,
				    &flags);
	if (es8323->spk_ctl_gpio < 0) {
		...
		gpio_direction_output(es8323->spk_ctl_gpio,
				      !es8323->spk_gpio_level);
	}

	es8323->hp_ctl_gpio =
	    of_get_named_gpio_flags(i2c->dev.of_node, "hp-con-gpio", 0, &flags);
	if (es8323->hp_ctl_gpio < 0) {
		...
		gpio_direction_output(es8323->hp_ctl_gpio,
				      !es8323->hp_gpio_level);
	}

	es8323->hp_det_gpio =
	    of_get_named_gpio_flags(i2c->dev.of_node, "hp-det-gpio", 0, &flags);
	if (es8323->hp_det_gpio < 0) {
		...
		if (hp_irq) {
			ret =
			    request_threaded_irq(hp_irq, NULL,
						 hp_det_irq_handler, irq_flag,
						 "ES8323", NULL);
			if (ret == 0) {
